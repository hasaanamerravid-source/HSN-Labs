/**
 * HSN Nexus — coupled EM + beam + iron + thermal + worldline tape.
 */
...
import {
  coupleStep, lawText, makeThermal,
...
<div class="lab-frame">
  ...
  <aside>
    ...
    <div class="section">
      ...
      <p class="help">
        ...
        and coil heat share one force law you can edit and rewind.
      </p>
    </div>
  </aside>
  <div id="viewport">
    <div class="overlay-hud" id="hud">Nexus · Lorentz + iron + Joule · worldline tape</div>
  </div>
  <div class="inspector wide-only">
    ...
    <div class="section">
      <h2>Coupling</h2>
      <p class="help" id="coupleTxt">Iron packing raises μ so the beam feels a stronger B. Heat detunes the RF kick.</p>
    </div>
  </div>
</div>
`;
...
export function mount(root, ctx) {
  ...
  const st = {
    useE: true, useB: true, useM: true, useCage: false, useBeamB: true, useHeat: true,
    B0: 0.8, kick: 8, spin: 0, phase: 0, cageA: 0,
    particles: [], irons: [],
    thermal: makeThermal(),
  ...
  function dropIron(n = 5) {
    ...
    for (let i = 0; i < n; i++) {
      let slot;
      if (st.irons.length >= cap) {
        slot = st.irons[(st.irons.length + i) % cap];
      } else {
        const mesh = new THREE.Mesh(ironGeo, new THREE.MeshPhongMaterial({ color: 0xb8c2cc, shininess: 50 }));
        irons.add(mesh);
        slot = { mesh, x: 0, y: 0.08, z: 0, vx: 0, vz: 0, chi: 2.2 };
        st.irons.push(slot);
      }
      const ang = Math.random() * Math.PI * 2;
      const r = 0.55 + Math.random() * 0.7;
      slot.x = Math.cos(ang) * r;
      slot.y = 0.08 + Math.random() * 0.04;
      slot.z = Math.sin(ang) * r;
      slot.vx = 0;
      slot.vz = 0;
      slot.chi = 2.2;
    }
    toast(st.irons.length >= cap ? "Iron cap reached — recycled oldest scraps" : "Iron scraps on the pole face");
  }
  ...
  function frame() {
    ...
    st.uiAcc += dt;
    if (st.uiAcc > 0.1) { st.uiAcc = 0; hud(); if (paintWl) paintWl(); }
    controls.update();
