...
const FUELS = {
  ch4: { name: "Methane", stoich: 9.52, hf: 50.0, carbon: 1 },
  c3h8: { name: "Propane", stoich: 23.8, hf: 46.4, carbon: 3 },
  h2: { name: "Hydrogen", stoich: 2.38, hf: 120.0, carbon: 0 },
  c2h5oh: { name: "Ethanol", stoich: 14.3, hf: 26.8, carbon: 2 },
};
...
<div class="lab-frame">
  ...
  <div id="viewport">
    ...
    <div class="overlay-hud">Classroom burner — completeness is schematic</div>
  </div>
...
export function mount(root, ctx) {
  ...
  function adiabatic() {
    ...
    const lean = Math.min(1, 1 / Math.max(0.4, st.phi));
    const richPen = st.phi > 1 ? 1 / st.phi : 1;
    return 300 + f.hf * 38 * lean * richPen;
  }
  ...
  on($("btnLite"), "click", () => {
    if (st.phi < 0.55 || st.phi > 1.75) { toast("Outside flammability window"); return; }
    st.lit = true;
    toast("Ignited");
  });
  ...
  function frame(now) {
    ...
    const co2 = st.lit ? Math.min(1, 1 / Math.max(st.phi, 0.6)) * (FUELS[st.fuel].carbon > 0 ? 1 : 0) : 0;
    ...
    $("rxn").textContent = st.fuel === "h2" ? "2 H2 + O2 → 2 H2O" : f.name + " + air → CO2 + H2O (+ soot if rich)";
