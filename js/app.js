...
export const CATALOG = [
  ...
  },
  {
    ...
    theory: "Non-relativistic classroom cyclotron: ω = |q|B/m. Electron mass is scaled so the orbit is visible.",
  ...
  },
  {
    ...
    tag: "Coupled",
    accent: "#ff6ad5",
    blurb: "Beam, magnetized scraps, and coil heat share one editable force law. Rewind the tape.",
    detail: "Lorentz + iron + Joule · worldline",
    theory: "F = q(E + v × B) + ∇(m·B). Beam current adds a Biot–Savart kick on the scraps; coil heat detunes RF.",
  ...
  },
  {
    ...
    tag: "Nuclear",
    accent: "#7af0c8",
    blurb: "Point-kinetics core: six-factor k-eff, one delayed-neutron group, Doppler-like temperature feedback.",
    detail: "k-eff · delayed neutrons · scram",
    theory: "ṅ = ((k(1-β)−1)/Λ)n + λC. Teaching model only — not a licensed core.",
  ...
  },
  {
    ...
    theory: "Adiabatic flame temperature vs φ. Lean burns cooler; rich makes soot. Not a full kinetics mechanism.",
...
function showPage(id) {
  ...
  document.querySelectorAll(".nav-btn").forEach((b) => {
    b.classList.toggle("active", b.dataset.nav === id || (id === "lab" && b.dataset.nav === "labs"));
  });
...
function bindSettings() {
  ...
  const resetM = $("btnResetMissions");
  if (resetM) {
    resetM.addEventListener("click", () => {
      Bus.resetMissions();
      renderMissions();
      toast("Mission board reset");
    });
  }
}
